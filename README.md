...
<p align="center"><sub>Drawn to the real proportions and the app's own colours: the notch is 179 by 32 points, the dashboard 860 by 136.</sub></p>
...
| **Transient HUD** | Copy feedback, power connection, or a clipboard preview | Triggered by the related local event; the copy HUD lasts as long as **Settings ▸ Popups ▸ Copy popup duration** says — Brief 1.5s by default, Standard 2.5s, Long 4s — and the power HUD lasts 2.5 seconds | Times out, closes when the pointer leaves, or promotes into the dashboard when selected |
...
| **Focus** | Do Not Disturb state when it can be read, and how much rebuildable cache is sitting on disk | Toggle Do Not Disturb through Control Center, and move the safe part of your caches to the Trash in one click | No named Focus profiles; toggling requires Accessibility permission, and the state reads as unknown until Control Center or Full Disk Access supplies it. Cache cleanup only offers folders it recognises, and the space is not free until you empty the Trash |
...
| **Cache cleanup** | Names and sizes of folders in `~/Library/Caches` and `~/Library/Logs`; listed developer caches with the switch on; sandboxed app containers only when Full Disk Access has already been granted | Moves folders it classifies as safe to the Trash when you press **Clean**, and remembers the last scan and clean in `UserDefaults` | No prompt is ever raised. Unidentified folders are never offered, and check-first folders are counted but never moved |
...
The current release is **[0.9.0](https://github.com/Srimi1/Notch-hub/releases/latest)** — `NotchHub-0.9.0-universal.dmg`, a universal build that runs on both Apple Silicon and Intel. Beside the app, the image carries a `Licenses` folder with every third-party license NotchHub redistributes.
...
```bash
shasum -a 256 -c NotchHub-0.9.0-universal.dmg.sha256
```

That prints `NotchHub-0.9.0-universal.dmg: OK`. Anything else means the file is not the one that was published — do not open it.
...
| Show cache cleanup in Focus | Enabled |
| Include developer caches | Disabled |
| Copy popup | Enabled |
...
- Cache cleanup offers only folders it can identify. Anything unrecognised, and anything classed as check-first — sign-in, iCloud, Spotlight, Mail, Photos, and sync caches — is never moved. Everything it does move goes to the Trash, so the space itself is reclaimed only when you empty it.
